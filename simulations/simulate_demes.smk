...
# rule run_ngsAMOVA_sfs_var_gtgl_forAmovaResults_forBenchmark:
	# input:
		# bcf="simulations/{simid}/model_{model_id}/contig_{contig}/vcfgl_var/{simid}-{model_id}-{contig}-rep{rep}-d{depth}.bcf",
		# metadata="simulations/{simid}/model_{model_id}/{simid}_{model_id}_metadata.tsv",
	# output:
		# amv="simulations/{simid}/model_{model_id}/contig_{contig}/doAmova1/{simid}-{model_id}-{contig}-rep{rep}-d{depth}_tole{tole}.amova.csv",
	# params:
		# outprefix="simulations/{simid}/model_{model_id}/contig_{contig}/doAmova1/{simid}-{model_id}-{contig}-rep{rep}-d{depth}_tole{tole}",
	# threads:
		# 1
	# log:
		# "simulations/{simid}/logs/model_{model_id}/contig_{contig}/doAmova1/{simid}-{model_id}-{contig}-rep{rep}-d{depth}_tole{tole}.amova.csv",
	# benchmark:
		# repeat("simulations/{simid}/benchmark/model_{model_id}/contig_{contig}/doAmova1/{simid}-{model_id}-{contig}-rep{rep}-d{depth}_tole{tole}.amova.csv",5)
	# shell:
		# """
		# (
		# {ngsAMOVA} -in {input.bcf} -isSim 1 -P {threads} -mEmIter 500000 -out {params.outprefix} -tole 1e-{wildcards.tole} -doAMOVA 1 -doDist 1 -m {input.metadata} -sqDist 1
		# )  2> {log}
		# """
#

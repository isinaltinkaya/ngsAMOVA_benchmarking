...
rule all:
	input:
		...
		simid=SIMULATION_ID,model_id=MODELS,contig=CONTIGS,depth=DEPTH,rep=REP),
		# expand("simulations/{simid}/model_{model_id}/contig_{contig}/call_genotype/postCutoff{postCutoff}_doPost{doPost}/ngsAMOVA/{simid}-{model_id}-{contig}-rep{rep}-d{depth}.amova.csv",	simid=SIMULATION_ID,model_id=MODELS,contig=CONTIGS,depth=DEPTH,rep=REP,postCutoff=postCutoffDict.keys(),doPost=[1, 2])
# # 		## 230227
# #		 ## GENOTYPE CALLING ###
# #		 230227 collect results
# #		 expand("simulations/sim_demes_v2/collected_results/collect_feb23/collect230227/sim_demes_v2_genotype_call_vc_doPost{doPost}_postCutoff{postCutoff}.csv",
# #			 doPost=[1,2],
# #			 postCutoff=postCutoffDict.keys()),
# #		 expand("simulations/sim_demes_v2/collected_results/collect_feb23/collect230227/sim_demes_v2_genotype_call_phi_doPost{doPost}_postCutoff{postCutoff}.csv",
# #			 doPost=[1,2],
# #			 postCutoff=postCutoffDict.keys()),
# #		 # 230227 run amova again with proper printing + save log
# #		 expand(
# #			 "simulations/{simid}/model_{model_id}/contig_{contig}/call_genotype/postCutoff{postCutoff}_doPost{doPost}/ngsAMOVA/{simid}-{model_id}-{contig}-rep{rep}-d{depth}.amova.csv",
# #			 simid=SIMULATION_ID,
# #			 model_id=MODELS,
# #			 contig=CONTIGS,
# #			 depth=DEPTH,
...
# #		 ## 230226
# #		 230226 run angsd again with proper wildcards
# #		 expand(
# #			 "simulations/{simid}/model_{model_id}/contig_{contig}/call_genotype/postCutoff{postCutoff}_doPost{doPost}/{simid}-{model_id}-{contig}-rep{rep}-d{depth}.bcf",
# #			 simid=SIMULATION_ID,
# #			 model_id=MODELS,
# #			 contig=CONTIGS,
# #			 depth=DEPTH,
...
rule angsd_call_genotypes_postCutoffvar_doPostvar:
	input:
	...
	output:
		"simulations/{simid}/model_{model_id}/contig_{contig}/call_genotype/postCutoff{postCutoff}_doPost{doPost}/{simid}-{model_id}-{contig}-rep{rep}-d{depth}.bcf"
	log:
		"simulations/{simid}/logs/model_{model_id}/contig_{contig}/call_genotype/postCutoff{postCutoff}_doPost{doPost}/{simid}-{model_id}-{contig}-rep{rep}-d{depth}.geno.gz",
	params:
		cutoffarg=lambda wildcards: str(postCutoffDict[str(wildcards.postCutoff)]),
		prefix="simulations/{simid}/model_{model_id}/contig_{contig}/call_genotype/postCutoff{postCutoff}_doPost{doPost}/{simid}-{model_id}-{contig}-rep{rep}-d{depth}",
	shell:
		"""
		({ANGSD} -doMajorMinor 3 -doMaf 1 -doGeno 31 -doPost {wildcards.doPost} -vcf-gl {input.vcf} -sites {input.sites} -out {params.prefix} -doBcf 1 {params.cutoffarg} )2>{log}
		"""

collect_results_genotype_call_doPost_postCutoff_vc = "/maps/projects/lundbeck/scratch/pfs488/AMOVA/simulations/AMOVA_paper_analyses/scripts/collect_results/collect_results_genotype_call_doPost_postCutoff_vc.sh"
...
rule collect_results_genotype_call_doPost_postCutoff_230227:
	input:
		expand("simulations/{simid}/model_{model_id}/contig_{contig}/call_genotype/postCutoff{{postCutoff}}_doPost{{doPost}}/ngsAMOVA/{simid}-{model_id}-{contig}-rep{rep}-d{depth}.amova.csv",
			simid=SIMULATION_ID,
			model_id=MODELS,
			contig=CONTIGS,
			depth=DEPTH,
	...
	output:
		vc="simulations/sim_demes_v2/collected_results/collect_feb23/collect230227/sim_demes_v2_genotype_call_vc_doPost{doPost}_postCutoff{postCutoff}.csv",
		phi="simulations/sim_demes_v2/collected_results/collect_feb23/collect230227/sim_demes_v2_genotype_call_phi_doPost{doPost}_postCutoff{postCutoff}.csv",
	shell:
		"""
		bash {collect_results_genotype_call_doPost_postCutoff_vc} {input} > {output.vc}

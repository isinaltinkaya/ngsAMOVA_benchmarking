...
df %>%
    ...
    theme_bw() +
    labs(
        x = "Depth", y = "RMSE (log10 transformed)",
        title = "RMSE of Phi Estimates",
        color = "Method",
        subtitle = model_id
    ) +
...
ggplot() +
    ...
    geom_line(data = rm_gle_phi, aes(
        x = Depth, y = Value, group = interaction(Contig, Level),
...
df %>%
    ...
    labs(
        title = paste0("RMSE of ", lut[method_id][[1]], " for ", phi_stat, " using ", model_id),
